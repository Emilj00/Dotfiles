...
local function download_lazygit()
    if vim.fn.filereadable(lazygit_path) == 1 then return end
    ...
    if arch == "x86_64" then arch = "x86_64" end -- standard
    ...
    local url = string.format(
        ...
        version, version, os_type, arch, extension
    )

    local download_cmd
    if is_windows then
        download_cmd = string.format("curl -L %s -o %s/lazygit.%s && tar -xf %s/lazygit.%s -C %s", 
            url, bin_dir, extension, bin_dir, extension, bin_dir)
    else
